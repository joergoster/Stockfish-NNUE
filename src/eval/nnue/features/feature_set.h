...
namespace Eval {
  namespace NNUE {
    namespace Features {
      ...
      template <typename T, T First, T... Remaining>
      constexpr std::array<T, sizeof...(Remaining) + 1> CompileTimeList<T, First, Remaining...>::kValues;
      ...
      struct InsertToSet<T, CompileTimeList<T, First, Remaining...>, AnotherValue> {
        using Result = std::conditional_t<CompileTimeList<T, First, Remaining...>::Contains(AnotherValue),
                                          CompileTimeList<T, First, Remaining...>,
                       std::conditional_t<(AnotherValue < First), CompileTimeList<T, AnotherValue, First, Remaining...>,
                                           typename AppendToList<T, typename InsertToSet<T, CompileTimeList<T, Remaining...>,
                                           AnotherValue>::Result, First>::Result>>;
      };
      ...
      public:
        ...
        static void AppendActiveIndices(const Position& pos, TriggerEvent trigger, IndexListType active[2]) {
        ...
        static void AppendChangedIndices(const PositionType& pos, TriggerEvent trigger,
          ...
          for (const auto perspective : Colors)
          {
              ...
              case TriggerEvent::kFriendKingMoved:
                  reset[perspective] = dp.pieceNo[0] == PIECE_NUMBER_KING + perspective;
                  break;
      ...
      class FeatureSet<FirstFeatureType, RemainingFeatureTypes...> :
        public FeatureSetBase<FeatureSet<FirstFeatureType, RemainingFeatureTypes...>> {
      ...
      public:
        // 評価関数ファイルに埋め込むハッシュ値
        static constexpr std::uint32_t kHashValue = Head::kHashValue ^ (Tail::kHashValue << 1) ^ (Tail::kHashValue >> 31);

        // 特徴量の次元数
        static constexpr IndexType kDimensions = Head::kDimensions + Tail::kDimensions;

        // 特徴量のうち、同時に値が1となるインデックスの数の最大値
        static constexpr IndexType kMaxActiveDimensions = Head::kMaxActiveDimensions + Tail::kMaxActiveDimensions;
      ...
      private:
        // 特徴量のうち、値が1であるインデックスのリストを取得する
        template <typename IndexListType>
        static void CollectActiveIndices(const Position& pos, const TriggerEvent trigger, const Color perspective,
