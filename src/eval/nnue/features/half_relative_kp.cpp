...
namespace Eval {
  namespace NNUE {
    namespace Features {
      ...
      inline IndexType HalfRelativeKP<AssociatedKing>::MakeIndex(Square sq_k, BonaPiece p) {
      ...
      // 駒の情報を取得する
      template <Side AssociatedKing>
      inline void HalfRelativeKP<AssociatedKing>::GetPieces(const Position& pos, Color perspective,
                                                            BonaPiece** pieces, Square* sq_target_k) {

        *pieces = (perspective == BLACK) ? pos.eval_list()->piece_list_fb()
                                         : pos.eval_list()->piece_list_fw();

        const PieceNumber target = (AssociatedKing == Side::kFriend) ? static_cast<PieceNumber>(PIECE_NUMBER_KING +  perspective)
      ...
      template <Side AssociatedKing>
      void HalfRelativeKP<AssociatedKing>::AppendActiveIndices(const Position& pos, Color perspective,
                                                               IndexList* active) {
        ...
        for (PieceNumber i = PIECE_NUMBER_ZERO; i < PIECE_NUMBER_KING; ++i)
            if (   pieces[i] != Eval::BONA_PIECE_ZERO
                && pieces[i] >= fe_hand_end)
                  active->push_back(MakeIndex(sq_target_k, pieces[i]));
      }

      // 特徴量のうち、一手前から値が変化したインデックスのリストを取得する
      template <Side AssociatedKing>
      void HalfRelativeKP<AssociatedKing>::AppendChangedIndices(const Position& pos, Color perspective,
